...
- **Tailwind config causes non-blocking build error**
  When building the application, we see an error:

  > Importing directly from a nuxt.config file is not allowed. Instead, use runtime config or a module.

  This error occurs because Tailwind is traversing `nuxt.config` directly into their own config. Overwriting this on our end seems not to work, but it seems also not to affect the build. See https://github.com/nuxt/framework/issues/2886.
