<template>
	...
	<span v-else-if="event.event.__typename === 'TokenModuleEvent'">
		<template v-if="event.event.eventType === 'revokeAdminRoles'">
			Revoked admin roles (<template v-for="(role, i) in event.event.details.revokeAdminRoles.roles" :key="role"><b>{{ role }}</b><template v-if="i < event.event.details.revokeAdminRoles.roles.length - 1">, </template></template>)
			for token <b>{{ event.tokenId }}</b> from
			<AccountLink :address="event.event.details.revokeAdminRoles.account.address" />
		</template>

		<template v-else-if="event.event.eventType === 'assignAdminRoles'">
			Assigned admin roles (<template v-for="(role, i) in event.event.details.assignAdminRoles.roles" :key="role"><b>{{ role }}</b><template v-if="i < event.event.details.assignAdminRoles.roles.length - 1">, </template></template>)
			for token <b>{{ event.tokenId }}</b> to
			<AccountLink :address="event.event.details.assignAdminRoles.account.address" />
		</template>

		<template v-else-if="event.event.eventType === 'removeAllowList'">
			Removed
			<AccountLink :address="event.event.details?.removeAllowList?.target?.address || event.event.details?.removeAllowList?.account?.address" />
			from allow list of token <b>{{ event.tokenId }}</b>
		</template>

		<template v-else-if="event.event.eventType === 'addAllowList'">
			Added
			<AccountLink :address="event.event.details?.addAllowList?.target?.address || event.event.details?.addAllowList?.account?.address" />
			to allow list of token <b>{{ event.tokenId }}</b>
		</template>

		<template v-else-if="event.event.eventType === 'removeDenyList'">
			Removed
			<AccountLink :address="event.event.details?.removeDenyList?.target?.address || event.event.details?.removeDenyList?.account?.address" />
			from deny list of token <b>{{ event.tokenId }}</b>
		</template>

		<template v-else-if="event.event.eventType === 'addDenyList'">
			Added
			<AccountLink :address="event.event.details?.addDenyList?.target?.address || event.event.details?.addDenyList?.account?.address" />
		...
		<template v-else>
			Token module event <b>{{ event.event.eventType }}</b> for token <b>{{ event.tokenId }}</b>
		</template>
	</span>
