...
$result = Get-PACertificate "*.${domain}" | 
...
az keyvault secret set --vault-name ccscan --name "${environmentName}-sslcert-pfx" --file $result.PfxFile --encoding base64 --not-before $result.NotBefore --expires $result.Expires --description "Base64 encoded PFX"
