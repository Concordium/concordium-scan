...
CREATE TABLE account_tokens (
    ...
    index
        BIGINT
        PRIMARY KEY,
  -- An account that owns a token balance.
    account_index
        BIGINT
        NOT NULL,
...
-- once the front-end queries a token by `token_address`).
CREATE INDEX token_holder_addresses_idx ON account_tokens (contract_index, contract_sub_index, token_id);
