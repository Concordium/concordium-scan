...
pub async fn run(
    ...
    tx.as_mut().execute(sqlx::raw_sql(include_str!("./m0006_canonical_address_and_transaction_search_index.sql"))).await?;



    let mut update_queries = {
        let mut accounts = sqlx::query(
            "SELECT index, address FROM accounts"
        )
            .fetch(tx.as_mut());
        ...
        while let Some(row) = accounts.try_next().await? {
            ...
            let account_address = concordium_rust_sdk::base::contracts_common::AccountAddress::from_str(&account_address)?;
            ...
            update_queries.push(sqlx::query(
                "UPDATE accounts SET canonical_address = $1 WHERE index = $2"
            )
