...
public class BlockRewardSpecialEvent : SpecialEvent
{
    ...
    public override IEnumerable<AccountBalanceUpdate> GetAccountBalanceUpdates()
    {
        if (FoundationCharge > CcdAmount.Zero)
            yield return new AccountBalanceUpdate(FoundationAccount, (long)FoundationCharge.Value, BalanceUpdateType.FoundationReward);
        if (BakerReward > CcdAmount.Zero)
            yield return new AccountBalanceUpdate(Baker, (long)BakerReward.Value, BalanceUpdateType.TransactionFeeReward);
    }
}