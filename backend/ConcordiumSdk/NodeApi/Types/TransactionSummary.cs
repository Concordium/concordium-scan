...
public class TransactionSummary
{
    ...
    public IEnumerable<AccountBalanceUpdate> GetAccountBalanceUpdates()
    {
        if (Sender != null && Cost > CcdAmount.Zero)
            yield return new AccountBalanceUpdate(Sender, -1 * (long)Cost.MicroCcdValue);
        ...
        if (Result is TransactionSuccessResult success)
        {
            ...
            foreach (var x in success.Events.OfType<Transferred>())
            {
                if (x.From is AccountAddress fromAccountAddress)
                    yield return new AccountBalanceUpdate(fromAccountAddress, -1 * (long)x.Amount.MicroCcdValue);
                if (x.To is AccountAddress toAccountAddress)
                    yield return new AccountBalanceUpdate(toAccountAddress, (long)x.Amount.MicroCcdValue);
            }
            ...
            foreach (var x in success.Events.OfType<TransferredWithSchedule>())
            {
                ...
                yield return new AccountBalanceUpdate(x.From, -1 * totalAmount);
                yield return new AccountBalanceUpdate(x.To, totalAmount);
            }

            foreach (var x in success.Events.OfType<ContractInitialized>())
                yield return new AccountBalanceUpdate(Sender!, -1 * (long)x.Amount.MicroCcdValue);

            foreach (var x in success.Events.OfType<Updated>())
            {
                if (x.Instigator is AccountAddress accountAddress)
                    yield return new AccountBalanceUpdate(accountAddress, -1 * (long)x.Amount.MicroCcdValue);
            }
        }
    }
}
