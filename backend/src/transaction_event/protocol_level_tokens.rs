...
use crate::{address::AccountAddress, transaction_event};

#[derive(Debug, Enum, Clone, Copy, PartialEq, Eq, sqlx::Type)]
#[sqlx(type_name = "event_type")] // only for PostgreSQL to match a type definition
...
#[derive(Debug, Enum, Clone, Copy, PartialEq, Eq, sqlx::Type)]
#[sqlx(type_name = "token_module_type")] // only for PostgreSQL to match a type definition
...
impl From<CborTokenAmount> for protocol_level_tokens::TokenAmount {
    fn from(cbor: CborTokenAmount) -> Self {
        ...
        let decimals = cbor.0.unsigned_abs() as u8;
        let value = cbor.1;
...
impl TokenUpdate {
    ...
    pub async fn save(
        ...
        println!("Saving token event: {:?}", self);
        ...
        match &self.event {
            TokenEventDetails::Module(e) => {
                println!("Handling token module event: {:?}", e.event_type.as_str());
        ...
        token_event = serde_json::to_value(self.event.clone()).unwrap_or(serde_json::Value::Null);
        sqlx::query!(
            "
        ...
        println!("amount change: {:?}", plt_amount_change);
        ...
        match event_type {
            ...
            TokenUpdateEventType::Transfer => {
                println!("Handling transfer event {:?}",self.event);
